...
class NavigationActivity : AppCompatActivity() {
    ...
    private fun setupWebView() {
        ...
        val home = prefs.getHomeAddress().ifEmpty { "Ljubljana, Slovenia" }
  ...
  function updatePosition(lat, lon, accuracy) {
    ...
    private fun startGps() {
        if (
            ...
            tvNavStatus.text = "GPS dovoljenje ni odobreno"
            return
        }
        ...
        try {
            locationManager?.requestLocationUpdates(
                ...
                tvNavStatus.text = "GPS aktiven"

                webView.evaluateJavascript(
                    "updatePosition(${loc.latitude}, ${loc.longitude}, ${loc.accuracy});",
                    null
                )
            }
        } catch (e: Exception) {
            tvNavStatus.text = "GPS napaka"
        }
    }
