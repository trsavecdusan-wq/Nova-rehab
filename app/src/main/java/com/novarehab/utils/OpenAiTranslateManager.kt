...
class OpenAiTranslateManager(private val context: Context) {
    ...
    fun translate(
        ...
        if (cleanText.isEmpty()) { onResult(""); return }
        if (targetLanguage == "sl") { onResult(cleanText); return }
        if (apiKey.isBlank()) { onResult(cleanText); return }

        val cacheKey = "${targetLanguage}_${cleanText.hashCode()}"
        prefs.getString(cacheKey, null)?.let { cached ->
            if (cached.isNotBlank()) { onResult(cached); return }
        }
    ...
    private fun translateWithOpenAi(text: String, targetLanguage: String, apiKey: String): String? {
        ...
        val messages = JSONArray()
            ...
            .put(JSONObject().put("role", "user").put("content", "Target language: $targetName\nText: $text"))
